...
## 从零到壹实现Marbles资产管理系统 （Fabric-SDK-Node）之－创建服务器端脚本

> 现在编写应用程序的WebSocket服务器端脚本，通过直接调用utils/marbles_cc_lib.js文件实现对事务的相关操作。该脚本的主要作用就是接收WebSocker客户端消息，并根据不同的请求做出相应的处理。

websocket_server_side.js：** WebSocket服务器端脚本，通过直接调用utils/marbles_cc_lib.js文件实现对事务的相关操作。

### websocker_server_side.js

在项目的utils目录下创建 websocket_server_side.js 文件并编辑
...
下一步：[web实现](https://github.com/kevin-hf/kevin-marbles/blob/master/md/12.%20web%E5%AE%9E%E7%8E%B0.md)