...
function connect_to_server() {
	...
	function onOpen(evt) {
		...
		addshow_notification(build_notification(false, '连接到Marbles应用程序'), false);
		connected = true;
	...
	function onClose(evt) {
		...
		setTimeout(() => {
			...
			connected = false;
			addshow_notification(build_notification(true, '与Marbles应用程序失去连接'), true);
...
function record_company(company) {
	...
	if (start_up === false) {
		...
		addshow_notification(build_notification(false, '发现一家新公司 "' + company + '"!'), true);
	}

	build_company_panel(company);
	if (start_up === true) addshow_notification(build_notification(false, '检测公司 "' + company + '".'), false);
